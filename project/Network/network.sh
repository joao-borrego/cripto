...
if [ `grep '^' /sys/class/net/enp0s3/address` = "08:00:43:53:43:21" ]
	then
		...
		sudo nmcli connection delete "eth0"
		sudo nmcli connection add type ethernet ifname enp0s3 con-name "Internet"
		sudo nmcli connection add type ethernet ifname enp0s8 con-name "eth0" ip4 192.168.1.2/24
...
if [ `grep '^' /sys/class/net/enp0s3/address` = "08:00:43:53:43:31" ]
 	then
		...
		sudo nmcli connection delete "eth0"
		sudo nmcli connection add type ethernet ifname enp0s3 con-name "Internet"
 		sudo nmcli connection add type ethernet ifname enp0s8 con-name "eth0" ip4 192.168.1.3/24
