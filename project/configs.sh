...
if [ `grep '^' /sys/class/net/enp0s3/address` = "08:00:43:53:43:31" ]
 	then
		# Change hostname
		sudo sed -i 's/machine1/idp1/g' /etc/hostname
		sudo sed -i 's/127.0.1.1	machine1/127.0.1.1	idp1/g' /etc/hosts
		# Add connections
		sudo nmcli connection delete "Wired connection 1"
		sudo nmcli connection delete "eth0"
...
if [ `grep '^' /sys/class/net/enp0s3/address` = "08:00:43:53:43:41" ]
	then
		# Change hostname
		sudo sed -i 's/machine1/wayf/g' /etc/hostname
		sudo sed -i 's/127.0.1.1	machine1/127.0.1.1	wayf/g' /etc/hosts
		# Add connections
		sudo nmcli connection delete "Wired connection 1"
		sudo nmcli connection delete "eth0"
...
if [ `grep '^' /sys/class/net/enp0s3/address` = "08:00:43:53:43:51" ]
	then
		# Change hostname
		sudo sed -i 's/machine1/idp2/g' /etc/hostname
		sudo sed -i 's/127.0.1.1	machine1/127.0.1.1	idp2/g' /etc/hosts
		# Add connections
		sudo nmcli connection delete "Wired connection 1"
		sudo nmcli connection delete "eth0"
